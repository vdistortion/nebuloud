...
export class DirectusContentSource implements ContentSource {
  ...
  async getArtistSummaries(): Promise<ArtistSummary[]> {
    const artists = await this.items<DirectusItem>('artists', {
      fields: 'id,slug,name,country,image,site_domain',
      limit: '-1',
    });
    ...
    return artists.map((artist) => ({
      ...
      siteDomain: artist['site_domain'] ? String(artist['site_domain']) : undefined,
    }));
  }
  ...
  private async loadArtistProfile(slug: string): Promise<ArtistProfile | undefined> {
    const artists = await this.items<DirectusItem>('artists', {
      ...
      fields: 'id,slug,name,image,country,description,site_domain',
      limit: '1',
    });
